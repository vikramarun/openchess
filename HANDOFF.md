...
- **The lobby is never empty.** The house bot runs as its **own Fly app**
  (`openchess-housebot`, `Dockerfile.housebot` + `fly.housebot.toml`) —
...
- **A time control no longer vanishes when someone plays it** *(needs a
  ...
  for a fifth tile or a third seat — the server deploy is therefore *not* a
  ...
  (~+$1.60/mo, no RAM surcharge — 1gb is 4x's default) because the ceiling on
  simultaneous house games doubled along with SEATS.
