...
impl Db {
    ...
    /// Atomically claim up to `limit` pending outbox rows (marks them
    /// `processing` so a second worker tick won't double-submit).
    pub async fn claim_settlements(&self, limit: i64) -> Result<Vec<OutboxRow>> {
        let rows = sqlx::query_as::<_, OutboxRow>(
            r#"UPDATE settlement_outbox SET status='processing', attempts=attempts+1
               WHERE id IN (
               ...
               RETURNING id, game_id, winner_addr"#,
        )
        .bind(limit)
        .fetch_all(&self.pool)
        .await?;
